from btadconfig import DefaultConfig
import os
...
class Model(object):
    def __init__(self, opt):
        ...
        self.save_root = f"./btad_result_test/{opt.model_name}_{opt.backbone_name}_k={opt.k}_cnum={opt.center_num}_sigma={opt.sigma}_inpaint_num={opt. in_lay_num}_clu_num={opt.clu_lay_num}/"
    ...
    def save_img(self, img_list, save_root, idx_name):
        ...
        save_root = os.path.join(save_root, idx_name[:4])
        os.makedirs(save_root, exist_ok=True)
        input_frame = denormalize(img_list[0].clone().squeeze(0).cpu().detach().numpy())
