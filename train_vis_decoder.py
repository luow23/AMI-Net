...
from vis_decoder import Decoder
...
class Model(object):
    def __init__(self, opt):
        ...
        if self.opt.isTrain:
            self.model.Roncon_model.eval()
            self.decoder.train()
            self.optimizer_g = optim.AdamW(self.decoder.parameters(), lr=opt.lr, betas=(0.9, 0.95))

        self.save_root = f"./result_decoder/{opt.model_name}_{opt.backbone_name}_k={opt.k}_cnum={opt.center_num}_sigma={opt.sigma}_inpaint_num={opt. in_lay_num}_clu_num={opt.clu_lay_num}_AvgTure_noise_scale20"       # os.makedirs(os.path.join(self.save_root, "weight"), exist_ok=True)
    ...
    def train(self):
        ...
        for epoch in range(0, 300):
            ...
            for index, (x,_,_, _) in enumerate(tqdm(self.trainloader, ncols=80)):
                ...
                rec_x = self.decoder(recon_feature)
                vis_loss = self.mseloss(x, rec_x)
                self.loss_scaler(vis_loss, self.optimizer_g, parameters=self.decoder.parameters(), update_grad=(index + 1) % 1 == 0)
                loss_total += vis_loss.item()
                count += 1
    ...
    def train_test(self):
        ...
        for idx, (x, y, mask, name) in enumerate(tqdm(self.testloader, ncols=80)):
            ...
            # ref_x = get_pos_sample(self.opt.referenc_img_file, self.device, 1)
            deep_feature, ref_feature, recon_feature, _, cos_sim, bin_mask = self.model(x, 'test')
            recon_x = self.decoder(recon_feature)
            cos_sim = cos_sim.view((1, int(64/opt.k),  int(64/opt.k)))
            cos_sim_cpu = cos_sim.cpu().detach().numpy().transpose((1, 2, 0))
            bin_mask = bin_mask.view((1,  int(64/opt.k),  int(64/opt.k)))
            bin_mask_cpu = bin_mask.cpu().detach().numpy().transpose((1, 2, 0))
            ...
            # amap = dir_amap + dis_amap
            self.vis_img(
                [x, recon_x, *feature_map_vis_list[2:], cos_sim_cpu, bin_mask_cpu, dis_amap, dir_amap, amap, amap, mask_cpu],
    ...
    def vis_img(self, img_list, save_root, idx_name):
        ...
        recon_frame = denormalize(img_list[1].clone().squeeze(0).cpu().detach().numpy())
        cv2_recon = np.array(recon_frame, dtype=np.uint8)
        plt.figure()
        plt.subplot(121)
        plt.imshow(cv2_input)
        plt.axis('off')
        plt.subplot(122)
        plt.imshow(cv2_recon)
        plt.axis('off')
        # plt.subplot(253)
        ...
        # plt.axis('off')
        plt.savefig(os.path.join(save_root, idx_name), dpi=600)
        plt.close()
