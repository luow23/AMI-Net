...
class Model(object):
    def __init__(self, opt):
        ...
        self.save_root = f"./fewshot_result/{opt.model_name}_{opt.backbone_name}_k={opt.k}_cnum={opt.center_num}_sigma={opt.sigma}_inpaint_num={opt. in_lay_num}_clu_num={opt.clu_lay_num}_AvgTure_shot={opt.shot}"       # os.makedirs(os.path.join(self.save_root, "weight"), exist_ok=True)
...
if __name__ == '__main__':
    opt = DefaultConfig()
    from datasets.dataset import MVTecDataset, FewshotMVTecDataset
    ...
    for classname in MVTec_CLASS_NAMES:
        ...
        opt.train_dataset = FewshotMVTecDataset(dataset_path=opt.data_root, class_name=opt.class_name, is_train=True, k=opt.shot)
